...
def conv2d(x, filters, kernel_size, stride=1, padding="same",
    ...
    strides = [stride] * 2
    dilation_rate = [dilation_rate] * 2      #扩张率表示使用扩张卷积时的扩张率。如果是一个整数，则所有方向的扩张率相等。
    #另外， strides 不等于1 和 dilation_rate 不等于1 这两种情况不能同时存在。在卷积核之间插入rate个0扩大视野感受范围
...
# l2norm (not bacth norm, spatial normalization) 空间归一化
...
# multibox layer: get class and location predicitions from detection layer  两个conv 预测classes 和 location
